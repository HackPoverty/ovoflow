...
import {
  ...
  ChickenFeeding,
  ChickenStock,
} from "@/components/forms/farmer-journal";
...
} from "@/components/forms/farmer-journal/schema";
...
export default function FarmerJournal() {
  ...
  const { currentStepIndex, steps, isFirstStep, isLastStep, step, back, next } =
    useMutistepForm(formSteps);
        ...
        <main className="flex flex-1 flex-col overflow-hidden">
          ...
          <FormProvider {...methods}>
            <form className="flex flex-1 flex-col overflow-hidden" onSubmit={onSubmit}>
              ...
              <div className="grid grid-cols-2 gap-2 border-t-2 bg-base-100 px-6 py-4">
                <button
                  disabled={isFirstStep || isSubmitting}
                  className="btn-primary btn-outline btn flex-1"
                  onClick={onBack}
                  type="button"
                >
                  {t("back")}
                </button>
                <button disabled={isSubmitting} className="btn-primary btn flex-1" type="submit">
                  {isSubmitting ? (
