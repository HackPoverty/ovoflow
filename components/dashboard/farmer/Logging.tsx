...
export default function Logging() {
  const { data, isLoading, error } = useSWR(
    "farmer-dashboard-logging",
    getLastRecordTimestampOfCurrentFarmer,
  );
  ...
  return (
    <>
      ...
      {
        <Link
          className={`btn mx-6 my-2 ${disabled ? "btn-disabled" : "btn-primary"}`}
          href="/journal"
        >
          {t("finish daily journal")}
        </Link>
      }
    </>
  );
...
function Error() {
  ...
  return (
    <div className="sticky top-0 bg-error px-4 py-2">
...
function CompleteNotice() {
  const t = useTranslations("FarmerDashboard");
  return (
    <div className="sticky top-0 z-10 bg-success px-4 py-2 text-sm shadow-lg">
...
function OverdueNotice() {
  const t = useTranslations("FarmerDashboard");
  return (
    <div className="sticky top-0 z-10 bg-warning px-4 py-2 text-sm shadow-lg">
