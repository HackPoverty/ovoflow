...
export default function ChickenEggProduction() {
  ...
  const [small, medium, large, damaged] = watch([
    "fieldSmallEggs",
    "fieldMediumEggs",
    "fieldLargeEggs",
    "fieldDamagedEggs",
  ...
  return (
    <div className="grid grid-cols-2 gap-2 p-6">
      ...
      <Label<FarmerJournal> htmlFor="fieldLayFrequency" required>
        {t("lay frequency")}
      </Label>
      <NumberInput name="fieldLayFrequency" />
      {errors.fieldLayFrequency ? (
        <ErrorMessage className="col-span-2">{errors.fieldLayFrequency.message}</ErrorMessage>
      ) : null}

      <Label<FarmerJournal> htmlFor="fieldLayFrequencyIndustrySta" required>
        {t("lay frequency")} ({t("industry standard")})
      </Label>
      <NumberInput name="fieldLayFrequencyIndustrySta" />
      {errors.fieldLayFrequencyIndustrySta ? (
        <ErrorMessage className="col-span-2">
          {errors.fieldLayFrequencyIndustrySta.message}
        </ErrorMessage>
      ) : null}
    </div>
  );
