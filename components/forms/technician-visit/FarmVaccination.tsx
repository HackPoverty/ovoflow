...
export default function FarmVaccination() {
  ...
  const [given, vaccines, other] = watch([
    "fieldVaccineGiven",
    "fieldVaccinations",
    "fieldOtherVaccine",
  ]);
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="text-sm text-error">{errors.fieldVaccinations?.message}</div>
    </div>
  );
}

const CommonVaccineInputDialog = forwardRef<HTMLDialogElement, { onClose: () => void }>(
  function CommonVaccineInputDialog({ onClose }, ref) {
    ...
    return (
      <dialog className="modal" ref={ref}>
        <div className="modal-box">
          <h3 className="text-lg font-bold">{t("choose vaccines")}</h3>
          {vaccines.map(({ value, description }) => (
